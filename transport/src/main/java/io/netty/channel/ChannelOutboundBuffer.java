...
public final class ChannelOutboundBuffer {
    ...
    // 当调用addMessage()方法后，从原链表tailEntry到Entry(原链表tailEntry)节点
    // 都是未被刷新的数据
    ...
    // 通道缓存总数居，用来控制背压
    // 美新增一个Entry,其大小要加上Entry实例的大小(96B)和真实数据的大小
     ...
     * 每调用一次ctx.channel()。write
     * 最终都会触发addMessage()方法，并把数据加载tailEntry后面
     */
    public void addMessage(Object msg, int size, ChannelPromise promise) {
        ...
        if (tailEntry == null) {
            ...
            //如果链表不为空，把新的节点添加到链表尾部
            Entry tail = tailEntry;
            tail.next = entry;
        }
        tailEntry = entry;
        //unflushedEntry为空，表示调用addFlush()方法将链表之前的所有元素都已经全部加入了需要发送的节点
        if (unflushedEntry == null) {
            unflushedEntry = entry;
        }

        //修改通道缓存的大小，若缓存总书记据大小超过了高水位，则会触发fireChannelWritabilityChanged事件，进入背压
    ...
    /**
     * 移动unflushedEntry的位置，修改待发送Entry的个数
     * 并且设置每个entry为非取消状态
        ...
        if (entry != null) {
            ...
            do {
                ...
                if (!entry.promise.setUncancellable()) {
                     ...
                     * entry如果已经取消
                     * 则释放entry对应的内存
                     * 减小ChannelOutboundBuffer的大小
                     * 如果缓存总数据的大小低于低水位
                     * 则触发fireChannelWritabilityChanged
