...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
    ...
    private ChannelFuture doBind(final SocketAddress localAddress) {
        ...
        if (regFuture.isDone()) {
            ...
            // 注册成功后要绑定端口
            // 由NioEventLoop线程去异步执行，此时需要创建ChannelPromise对象【Promise模式】
    ...
    final ChannelFuture initAndRegister() {
        ...
        try {
            /**
             * 根据serverBootStrap.channel(NioServerSocketChannel.class)
             * 反射创建NioServerSocketChannel对象
             */
            channel = channelFactory.newChannel();
            /**
             * 初始化NioServerSocketChannel
             * 设置Channel参数，为Worker线程管理的SocketChannel准备好参数
             * 及Handler消息处理链
             * 具体的代码直接看ServerBootStrap
             */
            init(channel);
        } catch (Throwable t) {
            //初始化处理失败
            //创建DefaultChannelPromise实例，并设置异常返回
            if (channel != null) {
        ...
        /**
         * 调用了SingleThreadEventLoop的register()方法
         * 最终触发了AbstractUnsafe的register
         */
