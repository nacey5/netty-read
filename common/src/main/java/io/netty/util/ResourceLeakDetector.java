...
public class ResourceLeakDetector<T> {
    ...
    private static final class DefaultResourceLeak<T>
            ...
            if (TARGET_RECORDS > 0) {
                ...
                do {
                    ...
                    if (numElements >= TARGET_RECORDS) {
                        //backOffFactor 是用来计算是否替换的因子，元素阅越多，值越大，最大为30
                        final int backOffFactor = Math.min(numElements - TARGET_RECORDS, 30);
                        //有1/2^backOffFactor的概率不会执行此if代码块
                        if (dropped = PlatformDependent.threadLocalRandom().nextInt(1 << backOffFactor) != 0) {
                            //把之前链表的的头作为第二个元素赋值给新链表
        ...
        private String generateReport(TraceRecord oldHead) {
            ...
            //记录太长的话，丢弃部分记录，记录丢弃了多少记录
            final int dropped = droppedRecordsUpdater.get(this);
            int duped = 0;

            //每次链表在更新头部的时候，其pos=旧的pos+1
            //因为最新的链表头部分pos就是链表长度
    ...
    private static class TraceRecord extends Throwable {
        ...
        public String toString() {
            ...
            out: for (int i = 3; i < array.length; i++) {
                ...
                // Strip the noisy stack trace elements. 不必要的方法可以跳过
