...
abstract class PooledByteBuf<T> extends AbstractReferenceCountedByteBuf {
    ...
    private void init0(PoolChunk<T> chunk, ByteBuffer nioBuffer,
        ...
        //大内存块默认为16M，背分配给多个PooledByteBuf
        chunk.incrementPinnedMemory(maxLength);
        this.chunk = chunk;
        memory = chunk.memory;
        //将PooledByteBuf转换成ByteBuffer
        tmpNioBuf = nioBuffer;
        //内存分配器arena，PooledByteBuf是由Arena的分配器构建的
    ...
    public final ByteBuf capacity(int newCapacity) {
        ...
        // Reallocation required. point！！！Arena是底层分配堆外内存的
    ...
    final ByteBuffer _internalNioBuffer(int index, int length, boolean duplicate) {
        //获取读索引
        index = idx(index);
        //当duplicate为true的时候，在memory中创建新的字节缓冲区共享此缓冲区
        //为false，先从tmpNioBuf中获取，为空的时候，再去调用newInternalNioBuffer
