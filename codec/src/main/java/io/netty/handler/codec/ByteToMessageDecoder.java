...
public abstract class ByteToMessageDecoder extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof ByteBuf) {
            ...
            try {
                ...
                //在第一次解码的时候只需要把data赋给字节容器就ok了
                //不是第一次解码，则需要写入cumulation
               ...
               //从cumulation字节中解码出消息
                ...
                try {
                    ...
                    int size = out.size();
                    //firedChannelRead属性在channelReadComplete()方法中被调用
                    firedChannelRead |= out.insertSinceRecycled();
                    //遍历解码消息集合，转发消息到下一个handler处理器中
    ...
    protected void callDecode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
        try {
            ...
            while (in.isReadable()) {
                ...
                //解码字节buf中的数据为消息对象，并将其放入out中
                //如果解码器被从通道处理器上下文移除了，则处理移除事件
    ...
    final void decodeRemovalReentryProtection(ChannelHandlerContext ctx, ByteBuf in, List<Object> out)
        ...
        try {
            ...
            if (removePending) {
                //再去触发一次handler，也就是让后续的handler去处理
